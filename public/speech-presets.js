(() => {
  ...
  function readEqInputs() {
    ...
    return rows.map((row) => {
      ...
      return {
        name,
        frequency: Number(inputs[0]?.value || 0),
        gain: Number(inputs[1]?.value || 0),
        q: Number(inputs[2]?.value || 0),
      }
    })
  }

  function buildSuggestions() {
    const resultText = document.querySelector('.result-panel')?.textContent || ''
    const bands = readEqInputs()
    ...
    if (!resultText) {
      return ['먼저 15–30초 측정을 완료하면 선택한 프리셋과 실측 결과를 비교합니다.']
    }
    ...
    if (/클리핑|피크가 높/.test(resultText)) {
      suggestions.unshift('EQ보다 먼저 X32 입력 게인을 2dB 낮추고 같은 거리에서 재측정하세요.')
    }
    if (/입력 레벨이 낮|평균 음량이 낮/.test(resultText)) {
      suggestions.unshift('EQ보다 먼저 마이크 거리와 입력 게인을 확인하세요.')
    }

    if (selected === 'warm' && !/저중역|먹먹/.test(resultText)) {
      suggestions.push('따뜻함이 부족할 때만 120–180Hz를 +0.5~+1dB 범위에서 시험하세요.')
    }
    if (selected === 'clear' && !/고역.*강|치찰/.test(resultText)) {
      suggestions.push('명료도가 필요하면 2–3.2kHz를 +0.5~+1dB 범위에서 시험하세요.')
    }
    if (selected === 'natural') {
      suggestions.push('자연스러운 프리셋은 모든 밴드를 0dB에 가깝게 두고 문제 대역만 작게 조정합니다.')
    }
