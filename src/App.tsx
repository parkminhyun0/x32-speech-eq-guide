...
import type { AnalysisResult, AudioState, EqBand, Sample } from './types'
...
export default function App() {
  ...
  function finishMeasurement() {
    ...
    if (maxPeak >= 92) {
      ...
      recommendations.push('X32 입력 게인을 먼저 2~4dB 낮추세요.')
      score -= 22
    } else if (maxPeak < 25) {
      findings.push('입력 레벨이 낮아 작은 발음이 묻힐 수 있습니다.')
      recommendations.push('게인을 소폭 높이고 큰 발성의 Peak를 다시 확인하세요.')
      score -= 12
    }
    if (averageRms < 4) {
      findings.push('평균 음량이 낮거나 마이크 거리가 멀 수 있습니다.')
      recommendations.push('마이크 거리를 일정하게 유지해 재측정하세요.')
      score -= 15
    }
    if (averageBands[2] > averageBands[5] + 14) {
      findings.push('160~350Hz 저중역이 상대적으로 많습니다.')
      recommendations.push('Low Mid 220~320Hz를 -1~-2dB부터 시험하세요.')
      score -= 15
    }
    if (averageBands[6] + 8 < averageBands[2] && averageRms >= 4) {
      findings.push('2.8~5.6kHz 명료도 대역이 상대적으로 부족합니다.')
      recommendations.push('현재 2~4kHz 컷을 0dB 쪽으로 복원한 뒤 비교하세요.')
      score -= 12
    }
    if (averageBands[7] > averageBands[5] + 16) {
      findings.push('5.6~10kHz가 상대적으로 강합니다.')
      recommendations.push('High 또는 High Mid 부스트를 1~2dB 줄여보세요.')
      score -= 10
    }
    if (!findings.length) {
      findings.push('뚜렷한 과다 대역이나 클리핑 신호가 발견되지 않았습니다.')
      recommendations.push('현재 설정을 기준으로 객석 위치를 바꿔 비교 측정하세요.')
    }
  ...
  return (
    <main className="app-shell">
      <header className="hero">
        <div>
          <p className="eyebrow">X32 SPEECH EQ GUIDE · MVP 1.1</p>
          <h1>기준 곡선과 실측 음성을<br />한 화면에서 비교합니다.</h1>
          <p className="hero-copy">설교 음성 목표 범위, 30초 실측 평균, 현재 X32 EQ 보정 방향을 겹쳐 보고 조정 이유를 이해합니다.</p>
        </div>
      ...
      <section className="control-grid">
        <article className="panel microphone-panel">
          <div className="panel-heading">
            <div><span className="step">01</span><h2>30초 설교 음성 측정</h2></div>
          ...
          <div className="timer-card">
            ...
            <p>{isListening ? '평소 설교 속도와 음량으로 말씀해 주세요.' : result ? '측정 완료. 아래 곡선과 X32 설정을 대조하세요.' : '최소 5초, 권장 30초입니다.'}</p>
          </div>
      ...
      {result && (
        <section className="panel result-panel">
          <div className="panel-heading compact">
            <div><span className="step">03</span><h2>설교 음향 분석 결과</h2></div>
      ...
      <EqCurve measuredBands={result?.averageBands} eqBands={eqBands} lowCutEnabled={lowCutEnabled} lowCutFrequency={lowCutFrequency} />
      <ToneGuide />
      <SpeechPresetAdvisor result={result} eqBands={eqBands} />
      <MeasurementConfidence result={result} />
      <LiveIpadMonitor />

      <section className="panel recommendation-panel">
        <div className="panel-heading compact"><div><span className="step">06</span><h2>적용 원칙</h2></div></div>
        <div className="recommendations">
          <div><span>1</span><p><strong>게인을 먼저 확인</strong>EQ보다 클리핑과 입력 레벨을 먼저 안정화합니다.</p></div>
          <div><span>2</span><p><strong>한 번에 한 밴드</strong>작게 조정한 뒤 같은 문장으로 재측정합니다.</p></div>
          <div><span>3</span><p><strong>객석 청취로 확정</strong>휴대폰 분석은 후보이며 최종 판단은 실제 청취로 합니다.</p></div>
        </div>
      </section>
      ...
      <footer>기준 영역은 비교용이며 휴대폰 마이크와 공간 특성의 영향을 받습니다. 자동 설정 확정이나 믹서 자동 변경은 하지 않습니다.</footer>
    </main>
  )
}
